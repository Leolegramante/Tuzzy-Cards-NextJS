...
import {CreateSubCategoryState, handleSubCategoryForm} from "./actions";
...
export const CreateSubCategoryModal = ({isOpen, onCloseAction}: CreateSubCategoryModalProps) => {
    const [state, setState] = useState(initialState);
    const [isLoading, setIsLoading] = useState(false);

    const formAction = async (formData: FormData) => {
        setIsLoading(true);
        const result = await handleSubCategoryForm(formData);
        setState(result);
        setIsLoading(false);
